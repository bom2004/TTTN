...
import {
    ...
    createVNPayPaymentThunk,
    deleteVNPayHistoryThunk,
    continueVNPayPaymentThunk
...
const VNPayTopup: React.FC = () => {
    ...
    const getMembershipInfo = (total: number) => {
        if (!total || total < 10000000) return { label: 'Silver', color: 'text-gray-600', bg: 'bg-gray-200' };
        if (total < 50000000) return { label: 'Gold', color: 'text-yellow-700', bg: 'bg-yellow-200' };
        if (total < 150000000) return { label: 'Diamond', color: 'text-blue-700', bg: 'bg-blue-200' };
        return { label: 'Platinum', color: 'text-purple-700', bg: 'bg-purple-200' };
    };
    ...
    const handleTopup = async (): Promise<void> => {
        ...
        setLoading(true);
        try {
            const res = await dispatch(createVNPayPaymentThunk({
                amount: numAmount,
                userId: userId!
            })).unwrap();
    ...
    const handleDeleteHistory = async (id: string): Promise<void> => {
        ...
        try {
            const res = await dispatch(deleteVNPayHistoryThunk(id)).unwrap();
            if (res.success) {
                toast.success(res.message || "Đã xóa lịch sử");
            }
        } catch (err: any) {
            toast.error(err || "Lỗi khi xóa lịch sử");
        }
        setActiveMenu(null);
    };
    ...
    const handleContinuePayment = async (id: string) => {
        try {
            const res = await dispatch(continueVNPayPaymentThunk(id)).unwrap();
            if (res.success && res.paymentUrl) {
                window.location.href = res.paymentUrl;
            } else {
                toast.error(res.message);
            ...
            toast.error(err || "Lỗi khi tiếp tục thanh toán");
        }
    };
                    ...
                    <div className="bg-white rounded-[32px] shadow-2xl shadow-blue-900/10 border border-gray-100 overflow-hidden">
                            ...
                            <div className="flex items-center gap-6 relative z-10">
                                ...
                                <div className="flex-1">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div className={`${getMembershipInfo(userData.totalRecharged || 0).bg} ${getMembershipInfo(userData.totalRecharged || 0).color} px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-sm`}>
                                            Thành viên {getMembershipInfo(userData.totalRecharged || 0).label}
                                        </div>
                                    </div>
                        ...
                        <div className="p-8 space-y-8">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div className="p-4 bg-gray-50 rounded-2xl">
                                    <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Tổng tích lũy (VIP)</p>
                                    ...
                                    <div key={idx} className="p-4 bg-gray-50 rounded-2xl border border-transparent hover:border-gray-100 transition-all relative">
                                        <div className="flex justify-between items-start mb-2">
                                            ...
                                            <div className="flex flex-col items-end gap-2 pr-6">
                                                ...
                                                {item.status === 'pending' && (
                                                    <button
                                                        onClick={() => handleContinuePayment(item._id || item.id || '')}
