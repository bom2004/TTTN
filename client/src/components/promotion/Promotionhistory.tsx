...
import { fetchUserPromotionHistoryThunk, selectPromotionHistory, selectPromotionLoading } from '../../lib/redux/reducers/promotion';

const PromotionHistory: React.FC = () => {
    const dispatch = useAppDispatch();
    const navigate = useNavigate();

    const history = useAppSelector(selectPromotionHistory);
    ...
    useEffect(() => {
        if (token && userData) {
            ...
            if (userId) {
                dispatch(fetchUserPromotionHistoryThunk(userId));
            }
        } else if (!token) {
            navigate('/login');
        }
    }, [token, userData, navigate, dispatch]);
