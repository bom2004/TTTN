...
const BookingStaff: React.FC = () => {
    ...
    const statusOptions = [
        ...
        { label: 'Chờ duyệt', value: 'pending', icon: 'pending_actions' },
        { label: 'Xác nhận', value: 'confirmed', icon: 'task_alt' },
        { label: 'Đã Check-in', value: 'checked_in', icon: 'login' },
        { label: 'Đã Check-out', value: 'checked_out', icon: 'logout' },
        { label: 'Hoàn thành', value: 'completed', icon: 'verified' },
    ...
    return (
        <div className="p-8 bg-[#f8fafc] min-h-screen font-sans">
            ...
            {/* Search, Grouped neatly */}
            <div className="mb-6 flex flex-col md:flex-row gap-4 items-center">
                <div className="relative flex-1 w-full group">
                    <span className="material-symbols-outlined absolute left-5 top-1/2 -translate-y-1/2 text-slate-400 group-focus-within:text-[#003580] transition-colors">search</span>
                    <input 
                        type="text"
                        placeholder="Tìm theo Mã đơn, Tên khách hoặc SĐT..."
                        className="w-full pl-14 pr-6 py-4.5 bg-white rounded-3xl border border-slate-200 focus:border-[#003580] focus:ring-4 focus:ring-blue-100/50 outline-none transition-all font-bold text-slate-900 shadow-sm"
                        value={searchTerm}
            ...
            {/* Premium Status Bar (Segmented Control style) */}
            <div className="bg-white p-2 rounded-[28px] shadow-sm border border-slate-100 mb-10 overflow-x-auto no-scrollbar scroll-smooth">
                        ...
                        <button
                            ...
                            className={`flex items-center gap-4 px-6 py-4 rounded-2xl transition-all duration-300 relative group
                                ${statusFilter === opt.value 
                                    ? 'bg-[#003580] text-white shadow-2xl shadow-blue-900/30 -translate-y-0.5 scale-[1.02]' 
                                    : 'text-slate-500 hover:bg-slate-50 hover:text-[#003580]'
                                }`}
                        >
                            <div className={`p-2 rounded-xl transition-all ${statusFilter === opt.value ? 'bg-white/10' : 'bg-slate-100 group-hover:bg-blue-50'}`}>
                                <span className={`material-symbols-outlined text-[22px] block 
                                    ${statusFilter === opt.value ? 'text-white' : 'text-slate-400 group-hover:text-[#003580]'}`}>
                                    {opt.icon}
                                </span>
                            </div>
                            <div className="flex flex-col items-start leading-none pr-2">
                                <span className="text-[11px] font-[900] uppercase tracking-[0.05em] mb-1">{opt.label}</span>
                                <div className="flex items-center gap-1.5">
                                    <span className={`text-[10px] font-bold ${statusFilter === opt.value ? 'text-blue-200' : 'text-slate-400'}`}>
                                        {getStatusCount(opt.value)}
                                    </span>
                                    <span className={`text-[8px] font-medium uppercase tracking-tighter ${statusFilter === opt.value ? 'text-blue-300' : 'text-slate-300'}`}>
                                        đơn hàng
                                    </span>
                                </div>
                            </div>
                            {statusFilter === opt.value && (
                                <div className="absolute bottom-1.5 left-1/2 -translate-x-1/2 w-6 h-1 bg-[#febb02] rounded-full"></div>
                            )}
                        </button>
                    ))}
                </div>
